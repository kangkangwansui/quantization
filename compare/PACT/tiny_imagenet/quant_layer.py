...
class QuantizedConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.w_bit = wbits
        self.a_bit = abits
        self.quantize_w = TorchQuantize(self.w_bit)
        self.quantize_a = TorchQuantize(self.a_bit)
        self.weight_rescale = \
            np.sqrt(1.0 / (kernel_size ** 2 * in_channels)) if (self.w_bit == 1) else 1.0
        print("使用PACT量化方法")
    ...
    def show_params(self):
        print("正在训练PACT量化方法，权重bit为{}，激活bit为{}".format(self.w_bit,self.a_bit))
