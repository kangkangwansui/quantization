...
parser.add_argument('--quant', type=str, default="conv", help='')
...
parser.add_argument('--seed', default=22, type=int, nargs='+',help='seed for initializing training. ')
...
def main(args):
    ...
    for epoch in range(0, settings.EPOCH):
        epoch1.append(epoch)
        if epoch > args.warm:
        ...
        if not epoch % settings.SAVE_EPOCH:
            weights_path = "{}_{}_regular".format(args.net,"tiny_image")
            print('saving weights file to {}'.format(weights_path))
            torch.save(net.state_dict(), weights_path)
