...
class ResNext(nn.Module):
    ...
    def __init__(self, block, num_blocks, class_names=100):
        ...
        self.conv2 = self._make_layer(block, num_blocks[0], 16, 1)
        self.conv3 = self._make_layer(block, num_blocks[1], 32, 2)
        self.conv4 = self._make_layer(block, num_blocks[2], 64, 2)
...
def resnet20():
    model = ResNext(ResNextBottleNeckC, [3, 3, 3])
    return model


def resnet32():
    model = ResNext(ResNextBottleNeckC, [5, 5, 5])
    return model


def resnet44():
    model = ResNext(ResNextBottleNeckC, [7, 7, 7])
    return model


def resnet56():
    model = ResNext(ResNextBottleNeckC, [9, 9, 9])
    return model
def resnext50():
