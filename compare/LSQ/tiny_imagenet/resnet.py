...
def CONV(in_channel, out_channel, kernel_size, stride=1, groups=1,padding = 0,quant_mode="conv"):
    ...
    if quant_mode == "apot":
        return QuantConv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding, groups = groups,bias = True)
    else:
        return nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding, groups = groups,bias = True)
...
class ResNet_cifar100(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100,quant_mode = "conv"):
        super().__init__()
